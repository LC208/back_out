...
from base.serializers import CompanyPracticecontactserializer
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **_):
        ...
        req = requests.get(
            "https://open.istu.edu/course/view.php?id=146https://open.istu.edu/course/view.php?id=146%D1%8D%D1%82%D0%BE",
            headers,
        )
        src = req.text
        companies = []
        ...
        user = []
        sections = re.split(
            'li id="section-([1-9][0-9]?|0)', src
        )  # i-ая секция хранит всю информацию о i-ой компании
        ...
        for i in range(len(companies)):
            # name
            class_section_name = re.search(
                'class="sectionname"><span><a href="\S{,57}">[^<>]+', companies[i]
            )[0]
            section_plus_name = re.search("section-[1-9][0-9]?.+", class_section_name)[
                0
            ]
            names.append(
                section_plus_name.split(">")[1]
                if len((section_plus_name.split(">")[1]).split(" - ")) == 0
                else (section_plus_name.split(">")[1]).split(" - ")[0]
            )
            # img
            img_src = re.findall('<img src="\S+"', companies[i])[0]
            (
                images.append((re.findall('http\S+"', img_src)[0])[:-1])
                if not ("icon" in img_src)
                else images.append(None)
            )
            # agreements
            agreements_array = re.findall(
                "Договор с ИрНИТУ.+[0-9][0-9]\.[0-9][0-9]\.[0-9][0-9]", companies[i]
            )
            ...
            # sites
            sub_sites = re.findall(
                '<a class="aalink" onclick="" href="\S{46,48}"><img src="\S{65,67}" class="iconlarge activityicon" alt="" role="presentation" aria-hidden="true" /><span class="instancename">Веб-сайт',
                companies[i],
            )
            try:
                ...
                raw_site = re.findall(
                    'Нажмите на ссылку <a href="\S+" >\S+</a>', sub_src
                )
                (
                    urls.append((requests.get(link, allow_redirects=True)).url)
                    if len(raw_site) == 0
                    else urls.append((re.findall('http.+"', raw_site[0])[0])[:-1])
                )
            # users
            login_user = names[i]
            login_user = (
                self.translit(login_user)
                if len(re.findall('".+"', names[i])) == 0
                else self.translit(re.findall('".+"', names[i])[0])
            )
            login_user = (
            ...
            pract = [
                {
                    "name": names[i],
                    "faculty": 5,
                    "links": [{"type": "Веб-сайт", "url": urls[i]}],
                }
            ]
            if urls[i] is None:
            ...
            ser = CompanyPracticecontactserializer(data=data_set)
            if ser.is_valid():
                ser.save()
            else:
