...
class Command(BaseCommand):
    def handle(self,*args,**kwargs):
        ...
        for item in items:
            ...
            for link in links:
                clear_name = re.sub(r'^\d+(\.\d+)*\s+', '', link.text).split('/')[0].rstrip() #удаление номера направления и его описания в скобках
                ...
                inst_blank = soup_link.find_all('div',class_='eduprofile-item-subdname eduprofile-form-element')
                for blank in inst_blank:
                    current_inst = blank.find(class_='eduprofile-form-element-value').text
        ...
        for item in items:
            ...
            clear_name = item.find(class_='eduprofile-item-header').text.replace("\n", "").split('/')[0].rstrip()
            ...
            if 'abiturientu' in link:
                ...
                if len(inst) == 0:
                    continue
                output_m.append({'faculty':inst[0].id,'url': link, 'full_name': clear_name, 'education_level': 3})
